...
Для выполнения практики понадобится сервер (в инструкции используется имя `s2-deb`, вы можете использовать свои имена) и клиент(в инструкции используется имя `s1-alma`, вы можете использовать свои имена).

1. **Подключитеесь к серверу `s1-alma`**
...
```bash
sudo ssh sa@s2-deb -L 10631:localhost:631
```
...
Теперь доступ к серверу возможен по адресу http://s2-deb:631
...
```
Nmap scan report for s2-deb (172.16.110.132)
...
19. Откройте web-интерфейс сервиса CUPS на сервере s2 по адресу http://s2-deb:631
...
```
```
sa@s2-deb:~$ sudo lpstat -p -d
принтер Cups_PDF_fedora_59 свободен. Включен с момента Пт 24 ноя 2023 13:25:16
...
24. **Подключитеесь к серверу `s1`**. Установите пакет `system-config-printer`. 
...
31. Откройте в браузере web-интерфейс сервиса CUPS на сервере **s2** по адресу **http://s2-deb:631**
