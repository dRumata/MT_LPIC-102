# Практическая работа 04.5.  Настройка сервера печати. Добавление общего принтера

---

## Цель работы

Получить практические навыки настройки и управления системой печати в Linux на основе CUPS (Common Unix Printing System), научиться добавлять и конфигурировать принтеры, управлять очередью печати, организовывать доступ к сетевым принтерам и осуществлять мониторинг задач печати.
...
В ходе работы вы освоили настройку сервера печати CUPS, научились добавлять локальные и сетевые принтеры, управлять правами доступа, организовывать общий доступ к принтерам, использовать веб-интерфейс CUPS для администрирования и отслеживать статус заданий печати. Эти навыки позволят вам эффективно управлять печатью в корпоративной среде и обеспечивать надежное обслуживание печатающих устройств.

---

## Контрольные вопросы

1. Какой порт по умолчанию использует CUPS для прослушивания входящих соединений и как это проверить?
2. Какие файлы конфигурации CUPS отвечают за управление доступом и права на печать?
3. Как добавить в систему сетевой принтер и убедиться, что он доступен всем пользователям?
4. Какие команды применяют для управления очередью печати и просмотра статуса заданий (`lp*`, `cupsd`)?
5. Как настроить общий доступ к принтеру в сети и какие параметры в `cupsd.conf` за это отвечают?
